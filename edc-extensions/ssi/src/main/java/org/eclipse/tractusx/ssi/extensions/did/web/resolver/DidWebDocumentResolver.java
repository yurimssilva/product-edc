...
public class DidWebDocumentResolver implements DidDocumentResolver {
  ...
  public DidDocument resolve(Did did) {
    ...
    try (final Response response = client.newCall(request).execute()) {
      ...
      final List<PublicKey> keys =
          resolvedDocument.getPublicKeys().stream()
              .filter(k -> k.getType().equals(Ed25519VerificationKey2020.TYPE))
              .map(
                  org.eclipse.tractusx.ssi.extensions.did.web.resolver.Ed25519VerificationKey2020
                          .class
                      ::cast)
              .map(this::mapKey)
  ...
  private Ed25519VerificationKey2020 mapKey(
      org.eclipse.tractusx.ssi.extensions.did.web.resolver.Ed25519VerificationKey2020 key) {
