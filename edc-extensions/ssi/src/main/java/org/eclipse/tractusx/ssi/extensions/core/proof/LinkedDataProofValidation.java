...
public class LinkedDataProofValidation {
    ...
    public LinkedDataProofValidation(LinkedDataHasher hasher, LinkedDataTransformer transformer, LinkedDataVerifier verifier, LinkedDataSigner signer) {
        this.hasher = hasher;
    ...
    public Ed25519Proof createProof(VerifiableCredential verifiableCredential, Did verificationMethodId, byte[] signingKey) {
        var transformedData = transformer.transform(verifiableCredential);
        var hashedData = hasher.hash(transformedData);
        var signature = signer.sign(hashedData, signingKey);

        var proof = Ed25519Proof.builder()
                .created(new Date())
